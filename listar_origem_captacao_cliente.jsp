<%-- 
    Document   : listar_origem_captacao_cliente
    Created on : 07/04/2014, 10:40:22
    Author     : ramon
--%>
...
<!DOCTYPE html>
   ...
   <link href="estilo.css" rel="stylesheet" type="text/css">
        ...
        <title>Webtrans - Consulta de Captação de Cliente</title>
    ...
    function desativarBotoes() {
            ...
            function abrirCadastro() {
                window.location = "OrigemCaptacaoControlador?acao=novoCadastro&modulo=webtrans";
            }

            function excluir(id, nome) {
                if (confirm("Deseja excluir a origem de captação '" + nome + "' ?")) {
                    if (confirm("Tem certeza?")) {
                        window.open("OrigemCaptacaoControlador?acao=excluir&modulo=webtrans&idOrigemCaptacao=" + id + "&descricao=" + encodeURIComponent(nome),
...
<html>
    ...
    <body onload="desativarBotoes(), setDefault()">
        ...
        <table class="bordaFina" width="70%" align="center">
            <tr>
                <td width="82%" align="left"> <span class="style4">Consulta de Origem de Captacão</span></td>
                <td width="18%">
            <c:if test="${param.nivel >= 3}">
                <input name="cadastrar" type="button" class="inputbotao"  onclick="tryRequestToServer(function() {
                            abrirCadastro();
                        });" value="Novo Cadastro"/>
            </c:if> 
        </td>
...
<table class="bordaFina" width="70%" align="center">
    <tr>
    <form action="OrigemCaptacaoControlador?acao=listar&modulo=webtrans" id="formulario" name="formulario" method="post">
        ...
        <td width="15" class="CelulaZebra1"><input name="pesquisar" id="pesquisar" type="button" class="inputbotao" value="  Pesquisar  " onclick="tryRequestToServer(function() {
...
<table width="70%" border="0" cellpadding="2" cellspacing="1" class="bordaFina" align="center">
    ...
    <c:forEach var="origem" varStatus="status" items="${listaListOrigemCaptacao}">             
        <tr class="${(status.count % 2 == 0 ? 'CelulaZebra1' : 'CelulaZebra2')}" >                    

           <td><a href="javascript: tryRequestToServer(function(){window.location='OrigemCaptacaoControlador?acao=iniciarEditar&modulo=webtrans&idOrigemCaptacao=${origem.id}';});" class="linkEditar">${origem.descricao}</a></td>
                    
                    

            <td align="center">
        <c:if test="${param.nivel >= 4}">
            <a href="javascript: tryRequestToServer(function(){excluir( '${origem.id}','${origem.descricao}');});"> <img class="imagemLink" src="img/lixo.png"/> </a>
        </c:if>                        
        </td>
...
<form id="formularioAnt" name="formularioAnt" action="OrigemCaptacaoControlador?acao=listar&modulo=webtrans" method="post">
    <td width="15%" align="center">
        ...
        <input name="botaoAnt" type="button" class="inputbotao" id="botaoAnt" onclick="tryRequestToServer(function() {
                    document.formularioAnt.submit();
                });" value="ANTERIOR"/>
    </td>
</form>

 <form id="formularioProx" name="formularioProx"  action="OrigemCaptacaoControlador?acao=listar&modulo=webtrans" method="post">
    <td width="15%" align="center">
        <input type="hidden" name="limiteResultados" value="<c:out value='${param.limiteResultados}'/>">
        <input type="hidden" name="operadorConsulta" value="<c:out value='${param.operadorConsulta}'/>">
        <input type="hidden" name="campoConsulta" value="<c:out value='${param.campoConsulta}'/>">
        <input type="hidden" name="valorConsulta" value= "<c:out value='${param.valorConsulta}'/>">
        <input type="hidden" name="paginaAtual" value="<c:out value='${param.paginaAtual + 1}'/>">
        <input name="botaoProx" type="button" class="inputbotao" id="botaoProx" onclick="tryRequestToServer(function() {
