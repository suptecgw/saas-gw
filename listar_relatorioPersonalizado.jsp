<%-- 
    Document   : listar_relatorioPersonalizado
    Created on : 20/03/2015, 18:20:13
    Author     : anderson
--%>
...
<script type="text/javascript" language="JavaScript" charset="Iso-8859-1">
    ...
    function abrirCadastro(){
        window.location = "ReportBuilderControlador?acao=novoCadastro&modulo=${param.modulo != '' ? param.modulo : ''}";
    }          

    function excluir(idRelatorio){       
        if(confirm("Deseja excluir o relatório?" )){
            if(confirm("Tem certeza?" )){
                window.open("ReportBuilderControlador?acao=excluir&idRelatorio="+idRelatorio,"pop", "width=210, height=100");
            }
        }
    }
    
    function editar(idRelatorio){
        window.location = "ReportBuilderControlador?acao=iniciarEditar&idRelatorio="+idRelatorio+"&modulo=${param.modulo != '' ? param.modulo : ''}";
    }
...
<html>
    ...
    <head>
        ...
        <title>Webtrans - Consulta de Relatórios Personalizados</title>
    </head>
    <body onload="desativarBotoes(), setDefault()"  > 
        <input name="modulo" type="hidden" class="inputtexto"  id="modulo" value="${param.modulo}">
        <img src="img/banner.gif" width="30%" height="44">
        <table class="bordaFina" width="68%" align="center">
            <tr>
                <td width="82%" align="left"> <span class="style4">Consulta de Relatórios Personalizados</span></td>
                <td width="18%">
        ...
        <table class="bordaFina" width="68%" align="center">
            <tr>
                <form action="ReportBuilderControlador?acao=listar&modulo=${param.modulo}" id="formulario" name="formulario" method="post">
                    <td width="97" class="CelulaZebra1">
                        
                        <select name="campoConsulta"  id="campoConsulta" class="inputtexto">
                            <option value="nome_relatorio">Descrição</option>
                        </select>             
        ...
        <table width="68%" border="0" cellpadding="2" cellspacing="1" class="bordaFina" align="center">
            ...
            <c:forEach var="report" varStatus="status" items="${listaRelatorioPersonalizado}">             
                <tr class="${(status.count % 2 == 0 ? 'CelulaZebra1' : 'CelulaZebra2')}" >                    
                    
                    <td><a href="javascript: tryRequestToServer(function(){editar('${report.id}')});" class="linkEditar">${report.table.name}</a></td>
                    
                    <td align="center">                        
                        <c:if test="${param.nivel >= 4}">
                            <a href="javascript: tryRequestToServer(function(){excluir('${report.id}');});"> <img class="imagemLink" src="img/lixo.png" alt="" title="Excluir Relatório Personalizado"/> </a>
                        </c:if>
                    </td>
        ...
        <table class="bordaFina" width="68%" align="center" >
            <tr class="CelulaZebra1">
                ...
                <form id="formularioAnt" name="formularioAnt" action="ReportBuilderControlador?acao=listar&modulo=webtrans" method="post">
                ...
                <form id="formularioProx" name="formularioProx"  action="ReportBuilderControlador?acao=listar&modulo=webtrans" method="post">
                    <td width="15%" align="center">
                        <input type="hidden" name="limiteResultados" value="<c:out value='${param.limiteResultados}'/>">
                        <input type="hidden" name="operadorConsulta" value="<c:out value='${param.operadorConsulta}'/>">
                        <input type="hidden" name="campoConsulta" value="<c:out value='${param.campoConsulta}'/>">
