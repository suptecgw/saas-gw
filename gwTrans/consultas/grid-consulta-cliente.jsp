                                ...
                                <div class="conteudo-colunas">
                                    ...
                                    <c:forEach var="column" items="${tela.targetXml.column}" varStatus="rowStatus" >
                                        ...
                                        <c:if test="${column.visivel && podeAdicionar}">
                                            <div class="col-md-12" onclick="adicionarColuna('${column.name}', '${column.label}', this);event.preventDefault();event.stopPropagation();"><img src="${homePath}/assets/img/plus_col.png"><label name="${column.name}">${column.label}</label></div>
                                            </c:if>
                                        </c:forEach>
                                </div>
                            </th>
                    ...
                    <tr>
                        <td>
                            ...
                            <c:forEach var="col" items="${row.columns}" varStatus="colStatus" >
                                <input type="hidden"  name="hi_row_${col.nome}_${rowStatus.count}" id="hi_row_${col.nome}_${rowStatus.count}" value="${col}" />
                            </c:forEach>
                        </td>
                        <td>
                <center>
                    <img width="15px" height="15px" title="Editar" style="cursor: pointer;" onclick="checkSession(function () {
                                editar(${rowStatus.count});
                            }, false);" src="img/edit.png">
                </center>
                </td>
                <td>
                <center>
                    <img width="17px" height="17px" style="cursor: pointer;" title="Imagens de documentos" src="${homePath}/img/anexar_imagem.png"
                         onclick="checkSession(function () {
                                     visualizarDocumentos(${rowStatus.count});
                                 }, false);">
                </center>
                </td>
                <c:forEach var="col" items="${tela.columns}" varStatus="colStatus" >
