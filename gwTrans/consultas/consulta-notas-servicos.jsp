...
<html>
    ...
    <head>
        ...
        <title>GW Trans - Consulta de Nota de Serviço</title>
        ...
        <link href="${homePath}/gwTrans/consultas/css/consulta-notas-servicos.css?v=${random.nextInt()}" rel="stylesheet" type="text/css"/>
    ...
    <body>
        ...
        <div id="topo">
            <div id="logo">
                <h3>Consulta de Nota de Serviço <img src="${homePath}/assets/img/trans_white.png"></h3>
            </div>
            <div id="actions">
                    ...
                    <c:if test="${nivelUser >= 3}">
                        <li>
                            <span class="bt bt-cadastro" onclick="checkSession(function () {
                                        btnCadastrar('cadvenda.jsp?acao=iniciar');
                                    }, false);">Novo Cadastro</span>
                        </li>
                        <c:if test="${utilizacaoNFSeG2ka eq false && stUtilizacaoNfse ne S}">
                            <li>
                                <span class="bt bt-cadastro" onclick="checkSession(function () {
                                            abrirPopConsulta('NFSeControlador?acao=listar&statusCte=P');
                                        }, false);">Visualizar NFS-e(s)</span>
                            </li>
                        </c:if>
                        <c:if test="${utilizacaoNFSeG2ka eq true}">
                            <li>
                                <span class="bt bt-cadastro" onclick="checkSession(function () {
                                            abrirNFSeG2ka();
                                        }, false);">Visualizar NFS-e(s) G2KA</span>
                            </li>
                        </c:if>
                    </c:if>
                </ul>
        ...
        <div id="sidebar" class="heightDoc">
            <div class="columnLeft" id="columnLeft">
                ...
                <div class="content">
                    <form action="ConsultaControlador?codTela=75" method="POST" id="formConsulta" name="formConsulta">
                        <div class="item_form">
                        ...
                        <div class="item_form div-ordenacao">
                            ...
                            <div id="filtros-adicionais-container" style="display: none;">
                                <div class="item_form check-box-feitos-por-mim" style="margin-top: 5px;">
                                    <label for="createdMe">
                                        <input type="checkbox" name="createdMe" id="createdMe">
                                        Criados por mim
                                    </label>
                                </div>
                                <div class="item_form check-box-apenas-nao-impressas" style="margin-top: 5px;">
                                    <label for="naoImpressas">
                                        <input type="checkbox" name="naoImpressas" id="naoImpressas">
                                        Mostrar Apenas as Notas Não Impressas
                                    </label>
                                </div>
                                <div class="item_form" style="margin-bottom: 50px;margin-top: 10px;">
                                    <select id="select-exceto-apenas-tipo-nota" name="select-exceto-apenas-tipo-nota">
                                        <option value="apenas">Apenas os Tipos</option>
                                        <option value="exceto">Exceto os Tipos</option>
                                    </select>
                                    <select id="select-tipo-nota" name="select-tipo-nota">
                                    </select>
                                </div>
                                <div class="item_form">
                                    <input type="text" id="inptSerie" name="inptSerie">
                                    <!--Necessario para que ele não de subimit sozinho com apenas um input text : Mateus--> 
                                    <input type="text" class="input-escondido" id="t">
                                </div>
            ...
            <input type="hidden" name="inp-auditoria" data-exclusao="true" data-rotina-auditoria="cadvenda">
        </div>
        <div id="map" class="heightDoc">
            ...
            <div id="footerTable">
                <div class="footerTableImpressao">
                    ...
                    <div class="footer02">
                        <label impressaoMatricial="true" style="margin-top: 5px !important;">Modelo</label>
                        <select name="cbmodelo" id="cbmodelo" >
                            <option value="1" <c:if test="${config.getRelDefaultConsultaVenda() == '1'}">selected</c:if>>Modelo 1</option>
                            <option value="2" <c:if test="${config.getRelDefaultConsultaVenda() == '2'}">selected</c:if>>Modelo 2</option>
                            <c:forEach items="${rels}" var="rel">
                                <c:set var="relCompleto" value="nota_servico_personalizado_${rel}" />
                                <option value="nota_servico_personalizado_${rel}" <c:if test="${config.getRelDefaultConsultaVenda() == relCompleto}">selected</c:if>>Modelo ${rel.toUpperCase()}</option>
                            </c:forEach>
                        </select>
                    </div>
                </div>
        ...
        <script src="${homePath}/gwTrans/consultas/js/consulta-notas-servicos.js?v=${random.nextInt()}"></script>
        ...
        <script defer type='text/javascript' lang="JavaScript">
                                ...
                                var menuMatricial = {
                                    nome: "Matricial de Venda",
                                    src: "${homePath}/assets/img/icones/print.png",
                                    acao: "popRel(this)"
                                };
                                addItemMenuGrid(menuMatricial);
            ...
            <c:if test="${pref != null}">
                ...
                <c:forEach var="filtros" items="${pref.filtros}" varStatus="filtrosStatus" >
                    <c:if test="${filtrosStatus.count == 1}">
                                    ...
                                    gerenciarStatusNota('${filtros.colunaNome}', '${filtros.colunaValor}', '${filtros.colunaCondicaoLocalizar}', '${filtros.descricao}');
                                    gerenciarInpSerie('${filtros.colunaNome}', '${filtros.colunaValor}');
