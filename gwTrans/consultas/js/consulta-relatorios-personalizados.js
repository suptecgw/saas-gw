...
function editar(position) {
        ...
        location.replace("ReportBuilderControlador?acao=iniciarEditar&idRelatorio=" +id+ "&modulo=webtrans");
}
...
function excluir(item) {
    ...
    var nomes = '';
        ...
        var texto = "";
        if (index > 1) {
            texto = "Deseja excluir os relatórios abaixo?";
        } else {
            texto = "Deseja excluir o relatório abaixo?";
        }
        chamarConfirm(texto, 'aceitouExcluirRelatorio("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');
    
}

function aceitouExcluirRelatorio(ids) {
    jQuery.ajax({
        url: 'ReportBuilderControlador?acao=excluir&idRelatorio=' + ids,
