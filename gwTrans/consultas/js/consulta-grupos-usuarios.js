...
function editar(position) {
    ...
    location.replace("GrupoUsuarioControlador?acao=iniciarEditar&idGrupo=" + id);
}
...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir os grupos de usuários abaixo?";
    } else {
        texto = "Deseja excluir o grupo de usuário abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirGrupo("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirGrupo(ids) {
    jQuery.ajax({
        url: 'GrupoUsuControlador?acao=excluir&ids=' + ids,
...
function gerenciarInpFilial(nome, valor, descricao, condicao) {
    if (nome == 'id_filial') {
        ...
        setTimeout(function () {
            ...
            while (valor[i]) {
                jQuery('#filtros-adicionais-container').show();
                addValorAlphaInput('inptFilial', descricao[i], valor[i]);
                i++;
            }
        }, 500);
    }
