...
function relatorioFuncao() {
    window.open('./relfuncaomaritimo.jsp?acao=iniciar&modulo=webtrans', "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=200,left=500,width=800,height=600");
}

function editar(position, podeexcluir) {
    var id = jQuery("#hi_row_id_" + position).val();
    location.replace("./cadfuncao.jsp?acao=editar&id=" + id);
}
...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir as funções abaixo?";
    } else {
        texto = "Deseja excluir a função abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirFuncao("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirFuncao(ids) {
    jQuery.ajax({
        url: 'FuncoesControlador?acao=excluir&ids=' + ids,
...
function salvarPreferencias() {
    ...
    jQuery.ajax({
        dataType: 'json',
        method: 'POST',
        url: 'ConsultaControlador?acao=salvarPreferencias&codigoTela=36',
        data: {'json': json},
        sucess: function () {}
    });
...
function salvarPesquisa() {
    if (iframeSalvarFiltros.document.getElementById('nomePesquisa').value.trim() === '') {
        ...
        jQuery.ajax({
            url: 'ConsultaControlador?acao=cadPrefUsuPersonalizada&nomePesquisa=' + nomePesquisa + "&isPrivado=" + isPrivado + '&aoSalvar=' + aoSalvar + '&idPreferencia=' + idPreferencia + '&cod_tela=36&nomePesquisaOriginal=' + nomePesquisaOriginal,
            type: 'POST',
            async: false,
