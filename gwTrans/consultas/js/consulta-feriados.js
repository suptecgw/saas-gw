...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir os feriados abaixo?";
    } else {
        texto = "Deseja excluir o feriado abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirFeriado("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirFeriado(ids) {
    jQuery.ajax({
        url: 'FeriadoControlador?acao=excluir&ids=' + ids,
...
function salvarPreferencias() {
    ...
    jQuery.ajax({
        dataType: 'json',
        method: 'POST',
        url: 'ConsultaControlador?acao=salvarPreferencias&codigoTela=96',
        data: {'json': json},
        sucess: function () {}
    });
...
function salvarPesquisa() {
    if (iframeSalvarFiltros.document.getElementById('nomePesquisa').value.trim() === '') {
        ...
        jQuery.ajax({
            url: 'ConsultaControlador?acao=cadPrefUsuPersonalizada&nomePesquisa=' + nomePesquisa + "&isPrivado=" + isPrivado + '&aoSalvar=' + aoSalvar + '&idPreferencia=' + idPreferencia + '&cod_tela=96&nomePesquisaOriginal=' + nomePesquisaOriginal,
            type: 'POST',
            async: false,
