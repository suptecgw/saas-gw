...
function cadastrarAeroporto() {
    location.replace("AeroportoControlador?acao=novoCadastro");
}

function relatorioAeroporto() {
    window.open('relaeroporto.jsp?acao=iniciar&modulo=webtrans', "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=200,left=500,width=800,height=600");
}

function editar(position) {
    var id = jQuery("#hi_row_id_" + position).val();
    location.replace("AeroportoControlador?acao=carregar&id=" + id);
}
...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir os aeroportos abaixo?";
    } else {
        texto = "Deseja excluir o aeroporto abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirAeroporto("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');
}

function aceitouExcluirAeroporto(ids) {
    jQuery.ajax({
        url: 'AeroportoControlador?acao=excluir&ids=' + ids,
