...
function relatorioPlano() {
    window.open('./relplanoconta.jsp?acao=iniciar&modulo=webtrans', "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=200,left=500,width=800,height=600");
}

function editar(position, podeexcluir) {
...
function aceitouExcluirPlano(ids) {
    jQuery.ajax({
        url: 'PlanoContaControlador?acao=excluir&ids=' + ids,
...
function salvarPreferencias() {
    ...
    jQuery.ajax({
        dataType: 'json',
        method: 'POST',
        url: 'ConsultaControlador?acao=salvarPreferencias&codigoTela=22',
        data: {'json': json},
        sucess: function () {}
    });
...
function salvarPesquisa() {
    if (iframeSalvarFiltros.document.getElementById('nomePesquisa').value.trim() === '') {
        ...
        jQuery.ajax({
            url: 'ConsultaControlador?acao=cadPrefUsuPersonalizada&nomePesquisa=' + nomePesquisa + "&isPrivado=" + isPrivado + '&aoSalvar=' + aoSalvar + '&idPreferencia=' + idPreferencia + '&cod_tela=22&nomePesquisaOriginal=' + nomePesquisaOriginal,
            type: 'POST',
            async: false,
...
function importarPlanoContas() {
    location.replace(homePath + '/PlanoContaControlador?acao=importarPlanoContas');
}
