...
function editar(position) {
    ...
    location.replace("./cadobservacao.jsp?acao=editar&id=" + id);
}
...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir as observações abaixo?";
    } else {
        texto = "Deseja excluir a observação abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirObservacao("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirObservacao(ids) {
    jQuery.ajax({
        url: 'ObservacaoControlador?acao=excluir&ids=' + ids,
