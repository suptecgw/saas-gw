...
function excluir(item) {
    ...
    var texto = "Deseja excluir o(s) conferentes(s) de carga abaixo?";
    chamarConfirm(texto, 'aceitouExcluirConferenteCarga("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirConferenteCarga(ids) {
    jQuery.ajax({
        url: 'ConferenteControlador?acao=excluir&ids=' + ids,
...
function salvarPesquisa() {
    if (iframeSalvarFiltros.document.getElementById('nomePesquisa').value.trim() === '') {
        ...
        jQuery.ajax({
            url: 'ConsultaControlador?acao=cadPrefUsuPersonalizada&nomePesquisa=' + nomePesquisa + "&isPrivado=" + isPrivado + '&aoSalvar=' + aoSalvar + '&idPreferencia=' + idPreferencia + '&cod_tela=' + codigo_tela + '&nomePesquisaOriginal=' + nomePesquisaOriginal,
            type: 'POST',
            async: false,
