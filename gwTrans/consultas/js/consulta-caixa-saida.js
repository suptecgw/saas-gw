...
jQuery(document).ready(function () {
    ...
    jQuery('#select-tipo-email').selectGrupoMultiploGw({
        input: 'select-tipo-email',
        grupos: {
            Status: {
                1: 'Pendente!!status!!Pendente!!IGUAL',
                2: 'Enviado!!status!!Enviado!!IGUAL',
                3: 'Não Enviado!!status!!Não Enviado!!IGUAL'
            }
        }
    });

    jQuery('#select-exceto-apenas-status-email').selectExcetoApenasGw({
        width: '170px'
    });
    ...
    setTimeout(function () {
        jQuery.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                removerValorInput('select-tipo-email');
                ...
                if (objCompleto.nomeStatus != null && objCompleto.nomeStatus != undefined) {
                    ...
                    for (var i = 0; i < valorStatus.length; i++) {
                        gerenciarMostrarEmails(objCompleto.nomeStatus, valorStatus[i], objCompleto.condicaoStatus, descStatus[i]);
                    }
                    jQuery('#filtros-adicionais-container').show();
                }
...
function gerenciarMostrarEmails(name, valor, condicao, descricao) {
    valor = valor.trim();
    if (name == 'status') {
        if (valor == 'Pendente') {
            jQuery(jQuery('.container-select-multiplo-gw-A').find('li')[1]).trigger('click');
            jQuery('#filtros-adicionais-container').show();
            jQuery('.container-li-valores').find(condicao === 'apenas' ? 'li:first-child' : 'li:last-child').trigger('click');
        } else if (valor == 'Enviado') {
            jQuery(jQuery('.container-select-multiplo-gw-A').find('li')[2]).trigger('click');
            jQuery('#filtros-adicionais-container').show();
            jQuery('.container-li-valores').find(condicao === 'apenas' ? 'li:first-child' : 'li:last-child').trigger('click');
        } else if (valor == 'Não Enviado') {
            jQuery(jQuery('.container-select-multiplo-gw-A').find('li')[3]).trigger('click');
            jQuery('#filtros-adicionais-container').show();
            jQuery('.container-li-valores').find(condicao === 'apenas' ? 'li:first-child' : 'li:last-child').trigger('click');
        }
    }
...
function enviarEmail(item) {
    ...
    if (item == undefined) {
        while (jQuery('input:checked[type=checkbox][name*=nCheck]')[index] != undefined) {
            ...
            var assunto = jQuery(jQuery('input:checked[type=checkbox][name*=nCheck]')[index]).parent().find('#hi_row_assunto_' + val).val();
        ...
        assuntos = jQuery('#hi_row_assunto_' + item).val();
    }

    var texto = "";
    if (index > 1) {
        texto = "Deseja enviar os emails abaixo?";
    } else {
        texto = "Deseja enviar o email abaixo?";
    }
    chamarConfirm(texto, 'aceitouEnviarEmail("' + ids + '")', '', 1, '<ul class="square">' + assuntos + '</ul>');
}

function aceitouEnviarEmail(ids) {
    jQuery('.bloqueio-tela').css('display', 'block');
    jQuery('.gif-bloq-tela').css('display', 'block');
    jQuery.ajax({
        url: 'CaixaSaidaControlador?acao=enviarEmails&ids=' + ids,
        success: function (data, textStatus, jqXHR) {
            reload();
        }
    });
