...
function editar(position) {
    ...
    location.replace("./cadnavio.jsp?acao=editar&id=" + id);
}
...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir os navios abaixo?";
    } else {
        texto = "Deseja excluir o navio abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirNavio("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirNavio(ids) {
    jQuery.ajax({
        url: 'NavioControlador?acao=excluir&ids=' + ids,
