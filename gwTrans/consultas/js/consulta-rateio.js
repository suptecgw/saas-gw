...
function cadastrarRateio() {
    location.replace("RateioControlador?acao=novoCadastro");
}

function relatorioRateio() {
    window.open('./relrateio.jsp?acao=iniciar&modulo=webtrans', "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=200,left=500,width=800,height=600");
}

function editar(position) {
    var id = jQuery("#hi_row_id_" + position).val();
    location.replace("RateioControlador?acao=iniciarEditar&idRateio=" + id);
}
...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir os rateios abaixo?";
    } else {
        texto = "Deseja excluir o rateio abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirRateio("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirRateio(ids) {
    jQuery.ajax({
        url: 'RateioControlador?acao=excluir&idRateio=' + ids,
...
function salvarPreferencias() {
    ...
    jQuery.ajax({
        dataType: 'json',
        method: 'POST',
        url: 'ConsultaControlador?acao=salvarPreferencias&codigoTela=18',
        data: {'json': json},
        sucess: function () {}
    });
...
function salvarPesquisa() {
    if (iframeSalvarFiltros.document.getElementById('nomePesquisa').value.trim() === '') {
        ...
        jQuery.ajax({
            url: 'ConsultaControlador?acao=cadPrefUsuPersonalizada&nomePesquisa=' + nomePesquisa + "&isPrivado=" + isPrivado + '&aoSalvar=' + aoSalvar + '&idPreferencia=' + idPreferencia + '&cod_tela=18&nomePesquisaOriginal=' + nomePesquisaOriginal,
            type: 'POST',
            async: false,
