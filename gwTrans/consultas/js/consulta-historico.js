...
function cadastrarHistorico() {
    location.replace("jspcadhistorico.jsp?acao=iniciar");
}

function relatorioHistorico() {
    window.open('./relhistorico.jsp?acao=iniciar&modulo=webtrans', "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=200,left=500,width=800,height=600");
}

function editar(position) {
    var id = jQuery("#hi_row_id_" + position).val();
    location.replace("./cadhistorico?acao=editar&id=" + id);
}
...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir os históricos abaixo?";
    } else {
        texto = "Deseja excluir o histórico abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirHistorico("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirHistorico(ids) {
    jQuery.ajax({
        url: 'HistoricoControlador?acao=excluir&ids=' + ids,
...
function salvarPreferencias() {
    ...
    jQuery.ajax({
        dataType: 'json',
        method: 'POST',
        url: 'ConsultaControlador?acao=salvarPreferencias&codigoTela=15',
        data: {'json': json},
        sucess: function () {}
    });
...
function salvarPesquisa() {
    if (iframeSalvarFiltros.document.getElementById('nomePesquisa').value.trim() === '') {
        ...
        jQuery.ajax({
            url: 'ConsultaControlador?acao=cadPrefUsuPersonalizada&nomePesquisa=' + nomePesquisa + "&isPrivado=" + isPrivado + '&aoSalvar=' + aoSalvar + '&idPreferencia=' + idPreferencia + '&cod_tela=15&nomePesquisaOriginal=' + nomePesquisaOriginal,
            type: 'POST',
            async: false,
