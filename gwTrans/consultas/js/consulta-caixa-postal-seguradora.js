...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir as Caixas Postais de Seguradoras abaixo?";
    } else {
        texto = "Deseja excluir a Caixa Postal de Seguradora abaixo?";
    }

    chamarConfirm(texto, 'aceitouExcluirCaixaPostalSeguradora("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');
}

function aceitouExcluirCaixaPostalSeguradora(ids) {
    jQuery.ajax({
        url: 'CaixaPostalSeguradoraControlador?acao=excluir&ids=' + ids,
