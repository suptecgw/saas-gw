...
function editar(position) {
        ...
        location.replace("BairroControlador?acao=iniciarEditar&idBairro="+id);
}
    ...
    var nomes = '';
        if (item == undefined) {
            while (jQuery('input:checked[type=checkbox][name*=nCheck]')[index] != undefined) {
                ...
                var nome = jQuery(jQuery('input:checked[type=checkbox][name*=nCheck]')[index]).parent().find('#hi_row_bairro_' + val).val();
            ...
            nomes = jQuery('#hi_row_bairro_' + item).val();
        }

        var texto = "";
        if (index > 1) {
            texto = "Deseja excluir os bairros abaixo?";
        } else {
            texto = "Deseja excluir o bairro abaixo?";
        }
        chamarConfirm(texto, 'aceitouExcluirBairro("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');
    
}

function aceitouExcluirBairro(ids) {
    jQuery.ajax({
        url: 'BairroControlador?acao=excluir&ids=' + ids,
        ...
        jQuery.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                if (objCompleto.nomeCidade != null && objCompleto.nomeCidade != undefined) {
                    ...
                    var nomeCid = objCompleto.descricaoCidade.replace(/\[+/g, '').replace(/\]+/g, '');
                    nomeCid = nomeCid.split(',');

                    for (var i = 0; i < cidade.length; i++) {
                        addValorAlphaInput('inptCidadeOri', nomeCid[i], cidade[i]);
                    }
                    jQuery('#filtros-adicionais-container').show();
                }
