...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir a(s) catagoria(s) de carga abaixo?";
    } else {
        texto = "Deseja excluir a(s) catagoria(s) de carga abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirCategoriaCarga("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirCategoriaCarga(ids) {
    jQuery.ajax({
        url: 'CategoriaCargaControlador?acao=excluir&ids=' + ids,
