...
function editar(position) {
    ...
    location.replace("./cadservico.jsp?acao=editar&id=" + id);
}
...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir os serviços abaixo?";
    } else {
        texto = "Deseja excluir o serviço abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirServico("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirServico(ids) {
    jQuery.ajax({
        url: 'ServicoControlador?acao=excluir&ids=' + ids,
