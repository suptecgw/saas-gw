...
function cadastrarTipoContainer() {
    location.replace("./cadtipo_container.jsp?acao=iniciar");
}

function relatorioTipoContainer() {
    window.open('reltipocontainer.jsp?acao=iniciar&modulo=webtrans', "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=200,left=500,width=800,height=600");
}

function editar(position) {
    var id = jQuery("#hi_row_id_" + position).val();
    location.replace("./cadtipo_container.jsp?acao=editar&id=" + id + "&ex=" + (nivelUser <= 3 ? 'false' : 'true'));
}
...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir os tipos de containers abaixo?";
    } else {
        texto = "Deseja excluir o tipo de container abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirTipoContainer("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');
}

function aceitouExcluirTipoContainer(ids) {
    jQuery.ajax({
        url: 'ContainerControlador?acao=excluir&ids=' + ids,
