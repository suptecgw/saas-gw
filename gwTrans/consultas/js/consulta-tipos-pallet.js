...
function editar(position) {
    ...
    location.replace("TipoPalletControlador?acao=iniciarEditar&id=" + id);
}
...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir os tipos de pallet abaixo?";
    } else {
        texto = "Deseja excluir o tipo de pallet abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirTipo("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirTipo(ids) {
    jQuery.ajax({
        url: 'TipoPalletControlador?acao=excluir&ids=' + ids,
...
function salvarPreferencias() {
    ...
    jQuery.ajax({
        dataType: 'json',
        method: 'POST',
        url: 'ConsultaControlador?acao=salvarPreferencias&codigoTela=35',
        data: {'json': json},
        sucess: function () {}
    });
...
function salvarPesquisa() {
    if (iframeSalvarFiltros.document.getElementById('nomePesquisa').value.trim() === '') {
        ...
        jQuery.ajax({
            url: 'ConsultaControlador?acao=cadPrefUsuPersonalizada&nomePesquisa=' + nomePesquisa + "&isPrivado=" + isPrivado + '&aoSalvar=' + aoSalvar + '&idPreferencia=' + idPreferencia + '&cod_tela=35&nomePesquisaOriginal=' + nomePesquisaOriginal,
            type: 'POST',
            async: false,
