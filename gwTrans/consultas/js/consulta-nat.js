...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir os NATs abaixo?";
    } else {
        texto = "Deseja excluir o NAT abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirNAT("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirNAT(ids) {
    jQuery.ajax({
        url: 'NatControlador?acao=excluir&ids=' + ids,
...
function salvarPreferencias() {
    ...
    jQuery.ajax({
        dataType: 'json',
        method: 'POST',
        url: 'ConsultaControlador?acao=salvarPreferencias&codigoTela=82',
        data: {'json': json},
        sucess: function () {}
    });
