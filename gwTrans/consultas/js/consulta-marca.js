...
function editar(position) {
    ...
    location.replace("./cadmarca?acao=editar&id=" + id);
}
...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir as marcas abaixo?";
    } else {
        texto = "Deseja excluir a marca abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirMarca("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirMarca(ids) {
    jQuery.ajax({
        url: 'MarcaControlador?acao=excluir&ids=' + ids,
