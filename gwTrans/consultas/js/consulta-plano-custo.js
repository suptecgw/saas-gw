...
function relatorioPlano() {
    window.open('./relplanocusto.jsp?acao=iniciar&modulo=webtrans', "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=200,left=500,width=800,height=600");
}

function editar(position) {
    var id = jQuery("#hi_row_id_" + position).val();
    location.replace("./cadplanocusto?acao=editar&id=" + id);
}
...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir os planos abaixo?";
    } else {
        texto = "Deseja excluir o plano abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirPlano("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirPlano(ids) {
    console.log("Chegou aqui")
    jQuery.ajax({
        url: 'PlanoCustoControlador?acao=excluir&ids=' + ids,
...
function salvarPreferencias() {
    ...
    jQuery.ajax({
        dataType: 'json',
        method: 'POST',
        url: 'ConsultaControlador?acao=salvarPreferencias&codigoTela=21',
        data: {'json': json},
        sucess: function () {}
    });
    ...
    setTimeout(function () {
        jQuery.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                jQuery('#ambosTipos').trigger('click');
                jQuery('#ambastrans').trigger('click');
                jQuery('#ambasprov').trigger('click');
                ...
                if (objCompleto.nomeTipo != null && objCompleto.nomeTipo != undefined) {
                    if (objCompleto.valorTipo == "Receita") {
                        jQuery('#receitas').prop('checked', true);
                        jQuery('#filtros-adicionais-container').show();
                    }

                    if (objCompleto.valorTipo == "Despesa") {
                        jQuery('#despesas').prop('checked', true);
                        jQuery('#filtros-adicionais-container').show();
                    }
                }

                if (objCompleto.nomeTransitoria != null && objCompleto.nomeTransitoria != undefined) {
                    if (objCompleto.valorTransitoria == "Sim") {
                        jQuery('#transitoria').prop('checked', true);
                        jQuery('#filtros-adicionais-container').show();
                    }

                    if (objCompleto.valorTransitoria == "Não") {
                        jQuery('#naoTransitoria').prop('checked', true);
                        jQuery('#filtros-adicionais-container').show();
                    }
                }

                if (objCompleto.nomeProvisao != null && objCompleto.nomeProvisao != undefined) {
                    if (objCompleto.valorProvisao == "Sim") {
                        jQuery('#provisao').prop('checked', true);
                        jQuery('#filtros-adicionais-container').show();
                    }

                    if (objCompleto.valorProvisao == "Não") {
                        jQuery('#naoProvisao').prop('checked', true);
                        jQuery('#filtros-adicionais-container').show();
                    }
                }

                if (objCompleto.nomeAtivo != null && objCompleto.nomeAtivo != undefined) {
                    if (objCompleto.valorAtivo == "Sim") {
                        jQuery('#ativos').prop('checked', true);
                        jQuery('#filtros-adicionais-container').show();
                    }

                    if (objCompleto.valorAtivo == "Não") {
...
function gerenciarRadioTipo(nome, valor) {
    if (nome == 'tipo') {
        if (valor == 'Receita') {
            jQuery('#receitas').trigger('click');
            jQuery('#filtros-adicionais-container').show();
        } else if (valor == 'Despesa') {
            jQuery('#despesas').trigger('click');
            jQuery('#filtros-adicionais-container').show();
        } else {
            jQuery('#ambosTipos').trigger('click');
        }
...
function gerenciarRadioTransitoria(nome, valor) {
    if (nome == 'transitoria') {
        if (valor == 'Sim') {
            jQuery('#transitoria').trigger('click');
            jQuery('#filtros-adicionais-container').show();
        } else if (valor == 'Não') {
            jQuery('#naoTransitoria').trigger('click');
            jQuery('#filtros-adicionais-container').show();
        } else {
            jQuery('#ambastrans').trigger('click');
        }
...
function gerenciarRadioProvisao(nome, valor) {
    if (nome == 'provisao') {
        if (valor == 'Sim') {
            jQuery('#provisao').trigger('click');
            jQuery('#filtros-adicionais-container').show();
        } else if (valor == 'Não') {
            jQuery('#naoProvisao').trigger('click');
            jQuery('#filtros-adicionais-container').show();
        } else {
            jQuery('#ambasprov').trigger('click');
        }
...
function gerenciarRadioAtivo(nome, valor) {
    if (nome == 'ativo') {
        if (valor == 'Sim') {
            ...
            jQuery('#ambasaativo').trigger('click');
        }
    }
}
...
function salvarPesquisa() {
    if (iframeSalvarFiltros.document.getElementById('nomePesquisa').value.trim() === '') {
        ...
        jQuery.ajax({
            url: 'ConsultaControlador?acao=cadPrefUsuPersonalizada&nomePesquisa=' + nomePesquisa + "&isPrivado=" + isPrivado + '&aoSalvar=' + aoSalvar + '&idPreferencia=' + idPreferencia + '&cod_tela=21&nomePesquisaOriginal=' + nomePesquisaOriginal,
            type: 'POST',
            async: false,
...
function sincronizarGwMobile(e, actionImport) {
    ...
    var planoCustoId = jQuery('#hi_row_id_' + ordem).val();
    window.open("./MobileControlador?acao=sincronizarGWMobilePlanoCusto&planoCustoId=" + planoCustoId + "&actionImport="+ actionImport, "pop", 'top=10,left=0,height=50,width=50,resizable=yes,status=1,scrollbars=1');
}
