...
function cadastrarEmbalagem() {
    location.replace("EmbalagemControlador?acao=novoCadastro&modulo=gwTrans");
}

function relatorioEmbalagem() {
    window.open('relembalagem.jsp?acao=iniciar&modulo=webtrans', "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=200,left=500,width=800,height=600");
}

function editar(position) {
    var id = jQuery("#hi_row_id_" + position).val();
    location.replace("EmbalagemControlador?acao=iniciarEditar&modulo=gwTrans&idEmbalagem=" + id);
}
...
function excluir(item) {
    ...
    chamarConfirm(texto, 'aceitouExcluirEmbalagem("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');

}

function aceitouExcluirEmbalagem(ids) {
    jQuery.ajax({
        url: 'EmbalagemControlador?acao=excluir&ids=' + ids,
...
function salvarPreferencias() {
    ...
    jQuery.ajax({
        dataType: 'json',
        method: 'POST',
        url: 'ConsultaControlador?acao=salvarPreferencias&codigoTela=37',
        data: {'json': json},
        sucess: function () {}
    });
...
function salvarPesquisa() {
    if (iframeSalvarFiltros.document.getElementById('nomePesquisa').value.trim() === '') {
        ...
        jQuery.ajax({
            url: 'ConsultaControlador?acao=cadPrefUsuPersonalizada&nomePesquisa=' + nomePesquisa + "&isPrivado=" + isPrivado + '&aoSalvar=' + aoSalvar + '&idPreferencia=' + idPreferencia + '&cod_tela=37&nomePesquisaOriginal=' + nomePesquisaOriginal,
            type: 'POST',
            async: false,
