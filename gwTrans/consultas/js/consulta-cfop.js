...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir os CFOP(s) abaixo?";
    } else {
        texto = "Deseja excluir o CFOP(s) abaixo?";
    }

    chamarConfirm(texto, 'aceitouExcluirCfop("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');
}

function aceitouExcluirCfop(ids) {
    jQuery.ajax({
        url: 'CfopControlador?acao=excluir&ids=' + ids,
...
function salvarPesquisa() {
    if (iframeSalvarFiltros.document.getElementById('nomePesquisa').value.trim() === '') {
        ...
        jQuery.ajax({
            url: 'ConsultaControlador?acao=cadPrefUsuPersonalizada&nomePesquisa=' + nomePesquisa + "&isPrivado=" + isPrivado + '&aoSalvar=' + aoSalvar + '&idPreferencia=' + idPreferencia + '&cod_tela=92&nomePesquisaOriginal=' + nomePesquisaOriginal,
            type: 'POST',
            async: false,
