...
function editar(position) {
    ...
    location.replace("CadastroControlador?codTela=78&modulo=editar&id=" + id+'&tema='+tema);
}
...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir as tabelas adicionais abaixo?";
    } else {
        texto = "Deseja excluir a tabela adicional abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirTabela("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');
}

function aceitouExcluirTabela(ids) {
    jQuery.ajax({
        url: 'TabelaAdicionalTdeControlador?acao=excluir&ids=' + ids,
...
jQuery(document).ready(function () {
    ...
    jQuery('#inptGrupoCli').inputMultiploGw({
        width: '96%',
        readOnly: 'true'
    });
    ...
    setTimeout(function () {
        jQuery.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                removerValorInput('inptGrupoCli');
                ...
                if (objCompleto.nomeGrupo != null && objCompleto.nomeGrupo != undefined) {
                    ...
                    var nomeGrupo = objCompleto.descricaoGrupo.replace(/\[+/g, '').replace(/\]+/g, '');
                    nomeGrupo = nomeGrupo.split(',');

                    for (var i = 0; i < grupo.length; i++) {
                        addValorAlphaInput('inptGrupoCli', nomeGrupo[i], grupo[i]);
                    }
                    jQuery('#filtros-adicionais-container').show();
                }
...
function gerenciarInpGrupoCliente(nome, valor, descricao, condicao) {
    if (nome == 'grupos_clientes_id') {
        ...
        setTimeout(function () {
            ...
            while (valor[i]) {
                jQuery('#filtros-adicionais-container').show();
                addValorAlphaInput('inptGrupoCli', descricao[i], valor[i]);
                i++;
            }
        }, 500);
    }
