...
function cadastrarUnidadeMedida() {
    location.replace("UnidadeMedidaControlador?acao=novoCadastro");
}

function relatorioUnidadeMedida() {
    window.open('relunidademedida.jsp?acao=iniciar&modulo=webtrans', "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=200,left=500,width=800,height=600");
}

function editar(position) {
    var id = jQuery("#hi_row_id_" + position).val();
    location.replace("UnidadeMedidaControlador?acao=iniciarEditar&modulo=gwFrota&idUnidadeMedida=" + id);
}
...
function excluir(item) {
    ...
    var texto = "";
    if (index > 1) {
        texto = "Deseja excluir as unidades de medidas abaixo?";
    } else {
        texto = "Deseja excluir a unidade de medida abaixo?";
    }
    chamarConfirm(texto, 'aceitouExcluirUnidadeMedida("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');
}

function aceitouExcluirUnidadeMedida(ids) {
    jQuery.ajax({
        url: 'UnidadeMedidaControlador?acao=excluir&ids=' + ids,
