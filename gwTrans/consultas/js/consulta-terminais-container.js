...
function cadastrarTerminalContainer() {
    location.replace("./cadterminal.jsp?acao=iniciar");
}

function relatorioTerminalContainer() {
    window.open('relterminal.jsp?acao=iniciar&modulo=webtrans', "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=200,left=500,width=800,height=600");
}

function editar(position) {
    var id = jQuery("#hi_row_id_" + position).val();
    location.replace("./cadterminal.jsp?acao=editar&id=" + id + "&ex=" + (nivelUser <= 3 ? 'false' : 'true'));
}
...
function excluir(item) {
    ...
    chamarConfirm(texto, 'aceitouExcluirTerminalContainer("' + ids + '")', '', 1, '<ul class="square">' + nomes + '</ul>');
}

function aceitouExcluirTerminalContainer(ids) {
    jQuery.ajax({
        url: 'terminalControlador?acao=excluir&ids=' + ids,
