...
//                    jQuery(ths[i]).css('max-width',w);
                ...
                if (jQuery('.conteudo-colunas')[0].innerHTML.trim() !== '') {
        ...
        <table cellspacing="0" class="tabela-gwsistemas"  id="tabela-gwsistemas">
            ...
            <tbody>
                    ...
                    <tr>
                        <td>
                            ...
                            <c:forEach var="col" items="${row.columns}" varStatus="colStatus" >
                                <input type="hidden" class="hi_row_${col.nome}_"  name="hi_row_${col.nome}_${rowStatus.count}" id="hi_row_${col.nome}_${rowStatus.count}" value="${col}" />
                            </c:forEach>
                        </td>
                        <td>
                <center>
                    <img class="img-menu-coleta" src="${homePath}/img/th-menu-escuro.png">
                    <nav class="container-menu-coleta">
                        <div class="topo-container-menu" menu-coleta="${rowStatus.count}">
                            <div style="max-width: 200px;text-align: center;overflow: hidden;text-overflow:ellipsis;white-space: nowrap;">

                            </div>
                        </div>
                        <div class="corpo-container-menu" ordem="${rowStatus.count}">
                            <ul>
                                <li class="${nivelUser < 1 ? 'disabled':''}" onclick="checkSession(function () {
                                    <c:if test="${nivelUser >= 1}">
                                            editar('${rowStatus.count}','${tema}');
                                    </c:if>
                                        }, false);"><img src="${homePath}/img/edit.png" width="20px"><span>Editar</span></li>

                                <li class="${nivelUser < 4 ? 'disabled':''}" onclick="checkSession(function () {
                                    <c:if test="${nivelUser >= 4}">
                                            excluir('${rowStatus.count}');
                                    </c:if>  
                                        }, false);"><img src="${homePath}/assets/img/icones/excluir.png" width="20px"><span>Excluir</span></li>
                            </ul>
                        </div>
                    </nav>
                </center>
                ...
                <center>
                    <c:if test="${nivelUser >= 1}">
                        <img width="15px" height="15px" title="Editar" style="cursor: pointer;" onclick="checkSession(function () {
                                    editar(${rowStatus.count},'${tema}');
                                }, false);" src="img/edit.png">
                    </c:if>
                </center>
                </td>
                <c:forEach var="col" items="${tela.columns}" varStatus="colStatus" >
                    ...
                    <c:if test="${indice > -1}">
                        <td>
                            <label title="${colResult.label}: ${colResult}" data-url-cadastro="${colResult.urlCadastro}" data-indice="${rowStatus.count}">${cg:formatCampoConsulta(colResult)}</label>
                        </td>
                    </c:if>
                </c:forEach>
                </tr>
...
<script type="text/javascript">
    ...
    function adicionarColuna(nomeColuna, labelColuna, elemento) {
        ...
        jQuery('.tabela-gwsistemas').trigger('coluna-adicionada');
    ...
    function criarMenuColunas() {
        ...
        div.append(jQuery(jQuery('.menu-span-dropdown-content')[0]).html());
        return div;
    }

    function criarMenuAddColunas() {
        var div = jQuery('<div class="conteudo-colunas">');
        div.append(jQuery(jQuery('.conteudo-colunas')[0]).html().trim());
        return div;
    }
        ...
        jQuery('.add-col').hover(
                function (e) {
                    ...
                    if (tamanhoTela - lugarHorizontal > tamanhoMenu) {
                        ...
                        if (tamanhoTela - lugarHorizontal > tamanhoSubMenu) {
                            jQuery(conteudoColunas).css('display', 'block');
                            jQuery(conteudoColunas).css('margin-left', w - 185 + 'px');
                        } else {
                            jQuery(conteudoColunas).css('display', 'block');
                            jQuery(conteudoColunas).css('margin-left', w - 345 + 'px');
                        }
...
//        });
        ...
        jQuery(document).on('event', '.evento-width', function () {
        });

        var c = 0;
        while (jQuery("[menu-coleta]")[c] !== undefined) {
            var nome = jQuery(jQuery('.tabela-gwsistemas tbody tr')[c]).find('label').html();
            var titulo = jQuery(jQuery('.tabela-gwsistemas thead tr')).find('label').html();
            var nomeCompleto = titulo + (": ") + nome;
            var index = jQuery(jQuery("[menu-coleta]")[c]).attr('menu-coleta');
            jQuery(jQuery("[menu-coleta]")[c]).find('div').html(jQuery(jQuery("[menu-coleta]")[c]).html() + nomeCompleto);
            c++;
        }
...
//                $($(a).parents("tr")).removeClass("datagrid-row datagrid-row-checked datagrid-row-selected").addClass("datagrid-row");
    ...
    function addItemMenuGrid(e) {
        var li = $('<li onclick="' + e.acao + '">');
        var img = $('<img src="' + e.src + '" width="20px">');
        var span = $('<span>').text(e.nome);

        li.append(img);
        li.append(span);
        
        $('.container-menu-coleta > .corpo-container-menu > ul').append(li);
    }
</script>
