<%-- 
    Document   : listar_UnidadeMedida
    Created on : 16/12/2008, 15:09:54
...
<script type="text/javascript" language="JavaScript" charset="Iso-8859-1">
    ...
    function abrirCadastro(){
        window.location = "LayoutEDIControlador?acao=novoCadastro&modulo=gwTrans";
    }          

    function excluir(id, nome){       
        if(confirm("Deseja excluir o layout '" + nome + "'?" )){
            if(confirm("Tem certeza?" )){
                window.open("LayoutEDIControlador?acao=excluir&modulo=gwTrans&idLayout=" + id + "&descricao=" + encodeURIComponent(nome),
...
<html>
    ...
    <head>
        ...
        <title>Webtrans - Consulta de Layout para EDI</title>
    ...
    <body onload="desativarBotoes(), setDefault()"  > 
        ...
        <table class="bordaFina" width="70%" align="center">
            <tr>
                <td width="82%" align="left"> <span class="style4">Consulta de Layout para EDI</span></td>
                <td width="18%">
        ...
        <table class="bordaFina" width="70%" align="center">
            <tr>
                <form action="LayoutEDIControlador?acao=listar&modulo=gwTrans" id="formulario" name="formulario" method="post">
                    <td width="97" class="CelulaZebra1">
                        ...
                        <select name="campoConsulta"  id="campoConsulta" class="inputtexto">
                            ...
                            <option value="extencao_arquivo">Extenção do Arquivo</option> 
                        </select>             
        ...
        <table width="70%" border="0" cellpadding="2" cellspacing="1" class="bordaFina" align="center">
            <tr>                
                ...
                <td class="tabela" width="20%" align="left">TIPO EDI</td>
                <td class="tabela" width="35%" align="left">NOME DO ARQUIVO</td>
                <td class="tabela" width="20%" align="left">EXTENÇÃO DO ARQUIVO</td>
                <td class="tabela" width="5%" align="left"></td>                
            </tr>
            <c:forEach var="layout" varStatus="status" items="${listaLayoutEDI}">             
                <tr class="${(status.count % 2 == 0 ? 'CelulaZebra1' : 'CelulaZebra2')}" >                    
                    
                    <td><a href="javascript: tryRequestToServer(function(){window.location='LayoutEDIControlador?acao=iniciarEditar&modulo=gwTrans&idLayout=${layout.id}';});" class="linkEditar">${layout.descricao}</a></td>
                    <td>${layout.tipoEdi=="c" ? "Conhecimento (CONEMB)": (layout.tipoEdi=="f" ? "Faturas (DOCCOB)" : layout.tipoEdi=="o" ? "Entrega (OCOREN)": "")}</td>
                    <td>${layout.nomeArquivo}</td>
                    <td>${layout.extencaoArquivo}</td>
                    <td align="center">
                        <c:if test="${param.nivel >= 4}">
                            <a href="javascript: tryRequestToServer(function(){window.location=excluir( '${layout.id}','${layout.descricao}');});"> <img class="imagemLink" src="img/lixo.png"/> </a>
                        </c:if>                        
                    </td>
        ...
        <table class="bordaFina" width="70%" align="center" >
            <tr class="CelulaZebra1">
                ...
                <form id="formularioAnt" name="formularioAnt" action="LayoutEDIControlador?acao=listar&modulo=gwTrans" method="post">
                ...
                <form id="formularioProx" name="formularioProx"  action="LayoutEDIControlador?acao=listar&modulo=gwTrans" method="post">
                    <td width="15%" align="center">
                        <input type="hidden" name="limiteResultados" value="<c:out value='${param.limiteResultados}'/>">
                        <input type="hidden" name="operadorConsulta" value="<c:out value='${param.operadorConsulta}'/>">
                        <input type="hidden" name="campoConsulta" value="<c:out value='${param.campoConsulta}'/>">
