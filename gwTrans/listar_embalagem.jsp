<%-- 
    Document   : listar_embalagem
    Created on : 15/10/2013, 17:01:45
    Author     : anderson
--%>
...
<script type="text/javascript" language="JavaScript" charset="Iso-8859-1">
    ...
    function abrirCadastro(){
        window.location = "EmbalagemControlador?acao=novoCadastro&modulo=gwTrans";
    }          

    function excluir(id){       
        if(confirm("Deseja excluir a Embalagem?" )){
            if(confirm("Tem certeza?" )){
                window.open("EmbalagemControlador?acao=excluir&modulo=gwTrans&idEmbalagem=" + id,
...
<html>
    ...
    <head>
        ...
        <title>Webtrans - Consulta de Embalagens</title>
    ...
    <body onload="desativarBotoes(), setDefault()"  > 
        ...
        <table class="bordaFina" width="68%" align="center">
            <tr>
                <td width="82%" align="left"> <span class="style4">Consulta de Embalagem</span></td>
                <td width="18%">
        ...
        <table class="bordaFina" width="68%" align="center">
            <tr>
                <form action="EmbalagemControlador?acao=listar&modulo=gwTrans" id="formulario" name="formulario" method="post">
        ...
        <table width="68%" border="0" cellpadding="2" cellspacing="1" class="bordaFina" align="center">
            <tr>                
                ...
                <td class="tabela" width="20%" align="left">Valor Unitario</td>
                <td class="tabela" width="5%" align="left"></td>                
            </tr>
            <c:forEach var="embalagem" varStatus="status" items="${listaEmbalagem}">             
                <tr class="${(status.count % 2 == 0 ? 'CelulaZebra1' : 'CelulaZebra2')}" >                    
                    
                    <td><a href="javascript: tryRequestToServer(function(){window.location='EmbalagemControlador?acao=iniciarEditar&modulo=gwTrans&idEmbalagem=${embalagem.id}';});" class="linkEditar">${embalagem.descricao}</a></td>
                    ...
                    <td align="center">
                        <c:if test="${param.nivel >= 4}">
                            <a href="javascript: tryRequestToServer(function(){excluir('${embalagem.id}');});"> <img class="imagemLink" src="img/lixo.png"/> </a>
                        </c:if>                        
                    </td>
        ...
        <table class="bordaFina" width="68%" align="center" >
            <tr class="CelulaZebra1">
                ...
                <form id="formularioAnt" name="formularioAnt" action="EmbalagemControlador?acao=listar&modulo=gwTrans" method="post">
                ...
                <form id="formularioProx" name="formularioProx"  action="EmbalagemControlador?acao=listar&modulo=gwTrans" method="post">
                    <td width="15%" align="center">
                        <input type="hidden" name="limiteResultados" value="<c:out value='${param.limiteResultados}'/>">
                        <input type="hidden" name="operadorConsulta" value="<c:out value='${param.operadorConsulta}'/>">
                        <input type="hidden" name="campoConsulta" value="<c:out value='${param.campoConsulta}'/>">
