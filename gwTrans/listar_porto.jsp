<%-- 
    Document   : listar_porto
    Created on : 14/06/2013, 17:00:54
...
<script type="text/javascript" language="JavaScript" charset="Iso-8859-1">
    ...
    function abrirCadastro(){
        window.location = "PortoControlador?acao=novoCadastro&modulo=gwFrota";
    }          

    function excluir(id){       
        if(confirm("Deseja excluir o porto?" )){
            if(confirm("Tem certeza?" )){
                window.open("PortoControlador?acao=excluir&modulo=gwFrota&idPorto=" + id,
...
<html>
    ...
    <head>
        ...
        <title>Webtrans - Consulta de Porto</title>
    ...
    <body onload="desativarBotoes(), setDefault()"  > 
        ...
        <table class="bordaFina" width="60%" align="center">
            <tr>
                <td width="82%" align="left"> <span class="style4">Consulta de Porto</span></td>
                <td width="18%">
        ...
        <table class="bordaFina" width="60%" align="center">
            <tr>
                <form action="PortoControlador?acao=listar&modulo=gwFrota" id="formulario" name="formulario" method="post">
        ...
        <table width="60%" border="0" cellpadding="2" cellspacing="1" class="bordaFina" align="center">
            ...
            <c:forEach var="porto" varStatus="status" items="${listaPorto}">             
                <tr class="${(status.count % 2 == 0 ? 'CelulaZebra1' : 'CelulaZebra2')}" >                    
                    
                    <td><a href="javascript: tryRequestToServer(function(){window.location='PortoControlador?acao=iniciarEditar&modulo=gwFrota&idPorto=${porto.id}';});" class="linkEditar">${porto.descricao}</a></td>
                    
                    
                    <td align="center">
                        <c:if test="${param.nivel >= 4}">
                            <a href="javascript: tryRequestToServer(function(){excluir('${porto.id}');});"> <img class="imagemLink" src="img/lixo.png"/> </a>
                        </c:if>                        
                    </td>
        ...
        <table class="bordaFina" width="60%" align="center" >
            <tr class="CelulaZebra1">
                ...
                <form id="formularioAnt" name="formularioAnt" action="PortoControlador?acao=listar&modulo=gwFrota" method="post">
                ...
                <form id="formularioProx" name="formularioProx"  action="PortoControlador?acao=listar&modulo=gwFrota" method="post">
                    <td width="15%" align="center">
                        <input type="hidden" name="limiteResultados" value="<c:out value='${param.limiteResultados}'/>">
                        <input type="hidden" name="operadorConsulta" value="<c:out value='${param.operadorConsulta}'/>">
                        <input type="hidden" name="campoConsulta" value="<c:out value='${param.campoConsulta}'/>">
