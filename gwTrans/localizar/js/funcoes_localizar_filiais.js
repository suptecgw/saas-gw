function marcarItemAdicionar(e) {
    ...
    if (elemento.hasClass('selecionado')) {
        ...
        var div = elemento.find('.abreviatura');
...
function podeEscolher(clone) {
    ...
    while (jQuery('#topo-resultados-col2').find('li')[i]) {
        if (jQuery(jQuery('#topo-resultados-col2').find('li')[i]).find('input[type=hidden]').val() === jQuery(clone).find('input[type=hidden]').val()) {
            parent.chamarAlert('A filial "' + jQuery(jQuery('#topo-resultados-col2').find('li')[i]).find('.razaosocial').html() + '" já foi selecionada.');
            return false;
        }
        i++;
    }
...
jQuery(document).ready(function () {
    ...
    if (jQuery('.opcoes-avancadas').attr('marcado') === 'false') {
        ...
        while (jQuery('#topo-resultados-col1 li')[i] !== undefined) {
            ...
            jQuery(jQuery('#topo-resultados-col1 li')[i]).attr('onclick', "parent.controleLocalizarFiliais('finalizado','" + abreviatura + "',true)");
            i++;
        }
    } else {
        var i = 0;
    ...
    jQuery('.fechar').click(function () {
        console.log("CHEGOU NO CLICK");
        jQuery('#topo-resultados-col2 li').remove();
        parent.controleLocalizarFiliais('fechar', null);
    });
...
function opcoesAvancadas() {
    ...
    if (elemento.attr('marcado') === 'false') {
        jQuery('.abreviatura').css('text-decoration', 'none');
        jQuery('.abreviatura').css('color', '#444');
        jQuery('.razaosocial').css('text-decoration', 'none');
        jQuery('.razaosocial').css('color', '#444');
        jQuery('.cnpj').css('text-decoration', 'none');
        jQuery('.cnpj').css('color', '#444');
        ...
        jQuery('.abreviatura').css('text-decoration', 'underline');
        jQuery('.abreviatura').css('color', '#13385c');
        jQuery('.razaosocial').css('text-decoration', 'underline');
        jQuery('.razaosocial').css('color', '#13385c');
        jQuery('.cnpj').css('text-decoration', 'underline');
        jQuery('.cnpj').css('color', '#13385c');
...
function enviarFiliaisSelecionadas() {
    var lis = jQuery('#topo-resultados-col2 li');

    if (lis.size() > 0) {
        parent.removerValorInput('inptFilial');
        var index = 0;
        while (lis[index]) {
            jQuery(lis[index]).find('.abreviatura').find('img').remove();
            var abreviatura = jQuery(lis[index]).find('.abreviatura').html();
            var id = jQuery(lis[index]).find('input').val();
            parent.controleLocalizarFiliais('finalizado', abreviatura + '!@!' + id);
            index++;
        }
    } else {
        parent.controleLocalizarFiliais('mensagem', 'Selecione ao menos uma Filial.');
    }
...
function popularListaFiliais(filial) {
    if (jQuery('#chkOpcoesAvancadas').prop('checked') == true) {
        jQuery('#topo-resultados-col1').append('<li class="ui-state-default" onclick="marcarItemAdicionar(this);"><div class="abreviatura">' + filial.abreviatura + '</div><div class="razaosocial">' + filial.razaosocial + '</div><div class="cnpj">' + filial.cnpj + '</div><div class="cidade">' + filial.cidade + '</div><div class="uf">' + filial.uf + '</div><input value="' + filial.id + '" type="hidden" name="idfilial"></li>');
        jQuery('.abreviatura').css('text-decoration', 'none');
        jQuery('.abreviatura').css('color', '#444');
    } else {
        jQuery('#topo-resultados-col1').append('<li class="ui-state-default" onclick="parent.controleLocalizarFiliais(' + "'" + "finalizado" + "'" + ',popularOnClick(this),true);"><div class="abreviatura">' + filial.abreviatura + '</div><div class="razaosocial">' + filial.razaosocial + '</div><div class="cnpj">' + filial.cnpj + '</div><div class="cidade">' + filial.cidade + '</div><div class="uf">' + filial.uf + '</div><input value="' + filial.id + '" type="hidden" name="idfilial"></li>');
        jQuery('.abreviatura').css('text-decoration', 'underline');
        jQuery('.abreviatura').css('color', '#13385c');
    }
}

function popularListaFiliaisEscolhidas(filial) {
    if (jQuery('#chkOpcoesAvancadas').prop('checked') == true) {
        jQuery('#topo-resultados-col2').append('<li class="ui-state-default" onclick="marcarItemAdicionar(this);"><div class="abreviatura">' + filial.split('#@#')[0] + '</div><input value="' + filial.split('#@#')[1] + '" type="hidden" name="idfilial"></li>');
        jQuery('.abreviatura').css('text-decoration', 'none');
        jQuery('.abreviatura').css('color', '#444');

    } else {
        jQuery('#topo-resultados-col2').append('<li class="ui-state-default" onclick="parent.controleLocalizarFiliais(' + "'" + "finalizado" + "'" + ',popularOnClick(this),true);"><div class="abreviatura">' + abreviatura + '</div><input value="' + id + '" type="hidden" name="idfilial"></li>');
        jQuery('.abreviatura').css('text-decoration', 'underline');
        jQuery('.abreviatura').css('color', '#13385c');
    }
}
...
function popularOnClick(element) {
    return jQuery(element).find('.abreviatura').html() + "!@!" + jQuery(element).find('input[name="idfilial"]').val();
}



function proximaLocalizar() {
    var filiais = jQuery('#inpt-filtrar-por').val();
    ...
    limparFiliaisResultado();

    jQuery.ajax({
        url: 'FilialControlador?acao=localizar&campo=' + campo + '&isDiferencia=' + check + '&isIgual=' + ckeckIgual + '&filiais=' + filiais + '&paginaAtual=' + paginaAtual + '&totalPaginas=' + totalPaginas,
        dataType: "text",
        method: "post",
        async: false,
        complete: function (jqXHR, textStatus) {
            ...
            var filiais = jqXHR.responseText.split('!@!');
            var i = 1;
            while (filiais[i] !== undefined) {
                var filial = jQuery.parseJSON(filiais[i]);
                if (filial.abreviatura !== undefined) {
                    popularListaFiliais(filial);
                }
                i++;
            }
        }
...
function localizarFiliais(filiais, campo, check, ckeckIgual) {
    limparFiliaisResultado();
    jQuery.ajax({
        url: 'FilialControlador?acao=localizar&filiais=' + filiais + '&isDiferencia=' + check + '&isIgual=' + ckeckIgual + '&campo=' + campo,
        dataType: "text",
        method: "post",
        async: false,
        complete: function (jqXHR, textStatus) {
