...
jQuery(document).ready(function () {
    ...
    if (jQuery('.opcoes-avancadas-grupo-cliente').attr('marcado') === 'false') {
        ...
        while (jQuery('#grupoCliente1 li')[i] !== undefined) {
            ...
            jQuery(jQuery('#grupoCliente1 li')[i]).attr('onclick', "parent." + controleLocalizar + "('finalizado','" + grupo + "')");
            i++;
        }
    } else {
        var i = 0;
    ...
    jQuery('#inpt-filtrar-por-vendedores').inputMultiploGw({
        width: '85%'
    });


    jQuery('#chkOpcoesAvancadasVendedores').click(function () {
        opcoesAvancadas();
    ...
    jQuery('.fechar').click(function () {
        if (tipoLocalizar == 'supervisor') {
            parent.controleLocalizarSupervisores('fechar', null);
        } else if (tipoLocalizar == 'vendedor') {
            parent.controleLocalizarVendedores('fechar', null);
        }
    });
...
function opcoesAvancadas() {
    ...
    if (elemento.attr('marcado') === 'false') {
        ...
        jQuery('.coluna-pesquisa').animate({
            ...
            jQuery('.topo-resultados-col2').css('margin-left', '2px');
            jQuery('#grupoCliente1 li, #grupoCliente2 li').css('width', '93%');
        ...
        jQuery('.coluna-pesquisa').animate({
            ...
            jQuery('.topo-resultados-col2').css('margin-left', '16px');
            jQuery('#grupoCliente1 li, #grupoCliente2 li').css('width', '95%');
            jQuery('.coluna-centro').animate({
                'width': '1%'
            }, velocidade, function () {
                jQuery('.coluna-centro').hide('fast');
...
//            var uf = jQuery(jQuery('#grupoCliente1 li')[i]).find('.estado').html();
            jQuery(jQuery('#grupoCliente1 li')[i]).attr('onclick', "parent."+controleLocalizar+"('finalizado','" + grupo + "',true)");
            i++;
        }
...
function enviarVendedoresSelecionadas() {
    var lis = jQuery('#grupoCliente2 li');

    if (tipoLocalizar == 'supervisor') {
        parent.removerValorInput(jQuery('input[name="inptSupervisor"]'));
    } else if (tipoLocalizar == 'vendedor') {
        parent.removerValorInput(jQuery('input[name="inptVendedor"]'));
    }
    ...
    if (lis.size() > 0) {
        ...
        while (lis[index]) {
            ...
            if (tipoLocalizar == 'supervisor') {
                parent.controleLocalizarSupervisores('finalizado', grupo + '!@!' + id);
            } else if (tipoLocalizar == 'vendedor') {
                parent.controleLocalizarVendedores('finalizado', grupo + '!@!' + id);
            }
            index++;
        }
    } else {
        if (tipoLocalizar == 'supervisor') {
            parent.controleLocalizarVendedores('mensagem', 'Selecione ao menos um supervisor.');
        } else if (tipoLocalizar == 'vendedor') {
            parent.controleLocalizarVendedores('mensagem', 'Selecione ao menos um vendedor.');
        }
    }
...
function popularOnClick(element) {
    return jQuery(element).find('.grupo').html() + '!@!' + jQuery(element).find('input').val();
}
