...
<script type="text/javascript" language="JavaScript" charset="Iso-8859-1">
    ...
    function abrirCadastro(){
        window.location = "UnidadeMedidaControlador?acao=novoCadastro&modulo=gwFrota";
    }          

    function excluir(id, nome){       
        if(confirm("Deseja excluir a Unidade de Medida '" + nome + "'?" )){
            if(confirm("Tem certeza?" )){
                window.open("UnidadeMedidaControlador?acao=excluir&modulo=gwFrota&idUnidadeMedida=" + id + "&descricao=" + encodeURIComponent(nome),
...
<html>
    ...
    <head>
        ...
        <title>Webtrans - Consulta de Unidades de Medidas</title>
    ...
    <body onload="desativarBotoes(), setDefault()"  > 
        ...
        <table class="bordaFina" width="70%" align="center">
            <tr>
                <td width="82%" align="left"> <span class="style4">Consulta de Unidades de Medidas</span></td>
                <td width="18%">
        ...
        <table class="bordaFina" width="70%" align="center">
            <tr>
                <form action="UnidadeMedidaControlador?acao=listar&modulo=gwFrota" id="formulario" name="formulario" method="post">
        ...
        <table width="70%" border="0" cellpadding="2" cellspacing="1" class="bordaFina" align="center">
            <tr>                
                <td class="tabela" width="20%" align="left" >SIGLA</td>
                <td class="tabela" width="75%" align="left">DESCRIÇÃO</td>
                <td class="tabela" width="5%" align="left"></td>                
            </tr>
            <c:forEach var="unidade" varStatus="status" items="${listaListUnidadeMedida}">             
                <tr class="${(status.count % 2 == 0 ? 'CelulaZebra1' : 'CelulaZebra2')}" >                    
                    
                    <td><a href="javascript: tryRequestToServer(function(){window.location='UnidadeMedidaControlador?acao=iniciarEditar&modulo=gwFrota&idUnidadeMedida=${unidade.id}';});" class="linkEditar">${unidade.sigla}</a></td>
                    <td>${unidade.descricao}</td>
                    
                    <td align="center">
                        <c:if test="${param.nivel >= 4}">
                            <a href="javascript: tryRequestToServer(function(){excluir( '${unidade.id}','${unidade.descricao}');});"> <img class="imagemLink" src="img/lixo.png"/> </a>
                        </c:if>                        
                    </td>
        ...
        <table class="bordaFina" width="70%" align="center" >
            <tr class="CelulaZebra1">
                ...
                <form id="formularioAnt" name="formularioAnt" action="UnidadeMedidaControlador?acao=listar&modulo=gwFrota" method="post">
                ...
                <form id="formularioProx" name="formularioProx"  action="UnidadeMedidaControlador?acao=listar&modulo=gwFrota" method="post">
                    <td width="15%" align="center">
                        <input type="hidden" name="limiteResultados" value="<c:out value='${param.limiteResultados}'/>">
                        <input type="hidden" name="operadorConsulta" value="<c:out value='${param.operadorConsulta}'/>">
                        <input type="hidden" name="campoConsulta" value="<c:out value='${param.campoConsulta}'/>">
